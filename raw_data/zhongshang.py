...
if __name__ == '__main__':
    ...
    for i in range(1,4):       # 爬取全部187页数据，设置为200页，确保都覆盖
        url = 'https://s.askci.com/stock/h/ci0000001208-0?reportTime=2023-12-31&pageNum={i}#QueryCondition'.format(i=i)
        page = ChromiumPage()
        page.get(url)
        data_list = []
        i = 0
        ...
        for tr_index in range(len(trs)):
            ...
            if row_data['行业分类'] == '汽车制造':
                ...
                # 格式化文件名
                file_name = '汽车-{}历史数据.xlsx'.format(row_data['股票简称'])

                # 使用 ExcelWriter 保存数据到指定的 Excel 文件和多个工作表
                with pd.ExcelWriter(file_name) as writer:
                    financial_df.to_excel(writer, index=False, sheet_name='{}资产负债表'.format(row_data['股票简称']))
                    profit_df.to_excel(writer, index=False, sheet_name='{}利润表'.format(row_data['股票简称']))
                    cashflow_df.to_excel(writer, index=False, sheet_name='{}现金流量表'.format(row_data['股票简称']))



    for stock_code, company_name in A_stock.items():
